...
import { formatUptime, formatBytes, batteryColor, tempColor, gradeColor, fmtClock, TZ_LABEL } from "@/lib/utils";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-2">
        <h1 className="font-display text-xl font-bold tracking-tight title-caret">Overview</h1>
        {data && (
          <span className="text-[10px] md:text-[11px] text-txt-muted data-value whitespace-nowrap">
            {fmtClock(data.timestamp)}{TZ_LABEL ? ` ${TZ_LABEL}` : ""}
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="h-full">
          {data?.battery.present ? <BatteryCard data={data} /> : <TailscaleCard data={data} />}
        </div>
        <div className="h-full">
          <ThermalsCard data={data} />
        </div>
        <div className="h-full">
...
function BatteryCard({ data }: { data: ReturnType<typeof useSystemData>["data"] }) {
  ...
  return (
    <Link href="/hardware" className="card px-4 py-3.5 group block h-full">
...
/* ---- Tailscale Connectivity Card ---- */
function TailscaleCard({ data }: { data: ReturnType<typeof useSystemData>["data"] }) {
  const ts = data?.tailscale;
  const up = ts?.online && ts?.state === "Running";
  return (
    <Link href="/network" className="card px-4 py-3.5 group block h-full">
      <div className="flex items-center justify-between mb-3">
        <span className="section-label">Tailscale</span>
        <ArrowIcon />
      </div>
      <div className="flex items-center gap-2 mb-2.5">
        <div className={clsx("status-dot", up ? "status-dot-online status-dot-pulse" : "status-dot-offline")} />
        <span className="text-sm font-semibold text-txt-primary">
          {!ts ? "—" : !ts.installed ? "Not installed" : up ? "Connected" : ts.state}
        </span>
      </div>
      <div className="grid grid-cols-2 gap-y-2.5 gap-x-4">
        <KV label="Peers online" value={ts ? `${ts.peersOnline} / ${ts.peersTotal}` : "—"} />
        <KV label="Route" value={!ts ? "—" : ts.relay ? `DERP ${ts.relay}` : "Direct"} />
      </div>
      {ts && ts.healthWarnings.length > 0 && (
        <div className="mt-2.5 pt-2 border-t border-line-dim">
          <span className="text-[10px] text-warning">{ts.healthWarnings.length} health notice{ts.healthWarnings.length > 1 ? "s" : ""}</span>
        </div>
      )}
    </Link>
  );
...
/* ---- Thermals Card ---- */
function ThermalsCard({ data }: { data: ReturnType<typeof useSystemData>["data"] }) {
  const t = data?.temperature;
  const celsius = t?.cpu;
  const headroom = celsius != null && t?.throttleAt ? Math.max(0, Math.round(t.throttleAt - celsius)) : null;
  return (
    <Link href="/hardware" className="card px-4 py-3.5 group block h-full">
      <div className="flex items-center justify-between mb-3">
        <span className="section-label">Thermals</span>
        <ArrowIcon />
      </div>
      <div className="flex items-center justify-between gap-3 mb-2.5">
        <div className="flex items-baseline gap-2">
          <span className="data-value text-3xl font-bold" style={{ color: tempColor(celsius ?? null) }}>
            {celsius !== null && celsius !== undefined ? <AnimatedNumber value={celsius} /> : "—"}
          </span>
...
function ServicesCard({ data }: { data: ReturnType<typeof useSystemData>["data"] }) {
  ...
  return (
    <Link href="/services" className="card px-4 py-3.5 group block h-full">
      <div className="flex items-center justify-between mb-3">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div>
          {/* Every scheduled job, not just the crontab lines. This tile read
              "13 Cron jobs" on a box running 60 scheduled jobs, because it
              counted the one mechanism the old collector could see. */}
          <div className="data-value text-lg font-bold text-txt-primary">{s?.scheduled.length ?? 0}</div>
          <div className="text-[10px] text-txt-muted">Scheduled jobs</div>
        </div>
        <div>
          <div className="data-value text-lg font-bold text-txt-primary">
            {s?.managed.filter((sv) => sv.state !== "failed").length ?? 0}
          </div>
          <div className="text-[10px] text-txt-muted">
            Services up
            {(s?.managed.filter((sv) => sv.state === "failed").length ?? 0) > 0 && (
              <span className="text-danger">
                {" "}· {s?.managed.filter((sv) => sv.state === "failed").length} failed
              </span>
            )}
          </div>
        </div>
...
function SecurityCard({ data }: { data: ReturnType<typeof useSystemData>["data"] }) {
  ...
  return (
    <Link href="/security" className="card px-4 py-3.5 group block h-full">
      <div className="flex items-center justify-between mb-3">
...
function UpdatesCard({ data }: { data: ReturnType<typeof useSystemData>["data"] }) {
  ...
  return (
    <div className="card px-4 py-3.5 h-full overflow-hidden">
      <div className="flex items-center justify-between mb-3">
      ...
      <div className="flex items-center gap-4">
        <div className="flex flex-col items-center shrink-0">
          <span className={clsx(
            ...
            <AnimatedNumber value={u?.available ?? 0} />
          </span>
...
/* ---- Helpers ---- */
function cpuSublabel(cpu: { cores: number; physicalCores: number | null }): string {
  if (cpu.physicalCores && cpu.physicalCores !== cpu.cores) {
    return `${cpu.physicalCores}C / ${cpu.cores}T`;
  }
...
function ArrowIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor"
      ...
      className="arrow-slide text-txt-muted group-hover:text-accent transition-colors">
